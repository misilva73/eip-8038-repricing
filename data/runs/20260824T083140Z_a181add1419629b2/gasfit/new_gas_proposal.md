...
_Generated 2026-08-24 09:55:50Z · fork `osaka` · anchor_rate 100 Mgas/s_

**Summary:** 12 parameters proposed — 8 increased, 4 decreased, 0 new, 0 unresolved · 3 warnings · 0 poor-fit selections
...
| --- | --- | --- | --- | --- |
| COLD_STORAGE_ACCESS | 2100 | 2055 | -45 | -2% |
| COLD_STORAGE_WRITE | 5000 | 16842 | +11842 | +237% |
| WARM_ACCESS | 100 | 32 | -68 | -68% |
| COLD_ACCOUNT_NOCODE_ACCESS | 2600 | 2038 | -562 | -22% |
| COLD_ACCOUNT_NOCODE_WRITE | 9300 | 33667 | +24367 | +262% |
| COLD_ACCOUNT_CODE_ACCESS | 2600 | 2746 | +146 | +6% |
| COLD_ACCOUNT_CODE_WRITE | 9300 | 8286 | -1014 | -11% |
| STORAGE_WRITE | 2800 | 14787 | +11987 | +428% |
| ACCOUNT_WRITE | 6700 | 31629 | +24929 | +372% |
| REFUND_STORAGE_CLEAR | 4800 | 16169 | +11369 | +237% |
| TX_ACCESS_LIST_STORAGE_KEY | 1900 | 2055 | +155 | +8% |
| TX_ACCESS_LIST_ADDRESS | 2400 | 2746 | +346 | +14% |
...
| --- | --- | --- | --- | --- | --- |
| COLD_STORAGE_ACCESS | geth | 2055 | besu | 1533 | 1.34× |
| COLD_STORAGE_WRITE | erigon | 16842 | geth | 11837 | 1.42× |
| WARM_ACCESS | nethermind | 32 | geth | 16 | 2.00× |
| COLD_ACCOUNT_NOCODE_ACCESS | geth | 2038 | besu | 1826 | 1.12× |
| COLD_ACCOUNT_NOCODE_WRITE | erigon | 33667 | besu | 14273 | 2.36× |
| COLD_ACCOUNT_CODE_ACCESS | geth | 2746 | erigon | 2683 | 1.02× |
| COLD_ACCOUNT_CODE_WRITE | geth | 8286 | besu | 7152 | 1.16× |
...
<details>
<summary><b>Priced glue opcodes with a poor fit</b> — 130 (glue_opcode, client) fits skipped</summary>
...
| `CALLDATASIZE` | `besu` (p-value), `erigon` (both), `ethrex` (R²), `geth` (R²) | — |
| `DIV` | `erigon` (both), `ethrex` (both), `geth` (both), `nethermind` (both), `reth` (both) | — |
| `DUP` | `besu` (p-value), `erigon` (both), `ethrex` (both), `geth` (both), `nethermind` (p-value), `reth` (p-value) | `COLD_ACCOUNT_NOCODE_ACCESS`, `COLD_ACCOUNT_NOCODE_WRITE`, `COLD_STORAGE_ACCESS`, `COLD_STORAGE_WRITE` |
| `EXP` | `nethermind` (both) | — |
| `GAS` | `besu` (p-value), `erigon` (both), `ethrex` (R²), `geth` (R²) | `COLD_ACCOUNT_CODE_ACCESS`, `COLD_ACCOUNT_CODE_WRITE`, `COLD_ACCOUNT_NOCODE_ACCESS`, `COLD_ACCOUNT_NOCODE_WRITE`, `COLD_STORAGE_ACCESS`, `WARM_ACCESS` |
...
| `MLOAD` | `besu` (p-value), `erigon` (both), `ethrex` (R²), `geth` (R²) | `COLD_ACCOUNT_NOCODE_ACCESS`, `COLD_ACCOUNT_NOCODE_WRITE`, `WARM_ACCESS` |
...
| `PUSH` | `erigon` (both), `ethrex` (both), `geth` (both), `nethermind` (p-value), `reth` (p-value) | `COLD_ACCOUNT_CODE_ACCESS`, `COLD_ACCOUNT_CODE_WRITE`, `COLD_ACCOUNT_NOCODE_ACCESS`, `COLD_ACCOUNT_NOCODE_WRITE`, `COLD_STORAGE_ACCESS`, `COLD_STORAGE_WRITE`, `WARM_ACCESS` |
| `PUSH0` | `besu` (p-value), `erigon` (both), `ethrex` (R²), `geth` (both) | — |
...
| `STATICCALL` | `besu` (p-value), `erigon` (both), `ethrex` (both), `geth` (R²) | — |
...
<details>
<summary><code>WARM_ACCESS</code> — 2 weak combos</summary>

| Test | Target opcode | Coef | Combo | Failing clients |
| --- | --- | --- | --- | --- |
| `test_ext_account_query_warm` | `EXTCODESIZE` | `target_coef` | `param_opcode=EXTCODESIZE` | `erigon` (R²), `geth` (R²), `reth` (R²) |
| `test_sload_same_key_benchmark` | `SLOAD` | `target_coef` | — | `reth` (R²) |

</details>

<details>
<summary><code>COLD_ACCOUNT_CODE_ACCESS</code> — 16 weak combos</summary>

| Test | Target opcode | Coef | Combo | Failing clients |
| --- | --- | --- | --- | --- |
| `test_account_access` | `BALANCE` | `target_coef` | `param_account_mode=AccountMode.EXISTING_CONTRACT_MINIMAL` / `param_opcode=BALANCE` | `geth` (R²) |
| `test_account_access` | `BALANCE` | `target_coef` | `param_account_mode=AccountMode.EXISTING_CONTRACT_SAME_MAX` / `param_opcode=BALANCE` | `geth` (R²) |
| `test_account_access` | `CALL` | `target_coef` | `param_account_mode=AccountMode.EXISTING_CONTRACT_MINIMAL` / `param_opcode=CALL` | `geth` (R²) |
| `test_account_access` | `CALL` | `target_coef` | `param_account_mode=AccountMode.EXISTING_CONTRACT_SAME_MAX` / `param_opcode=CALL` | `geth` (R²) |
| `test_account_access` | `CALLCODE` | `target_coef` | `param_account_mode=AccountMode.EXISTING_CONTRACT_MINIMAL` / `param_opcode=CALLCODE` | `geth` (R²) |
| `test_account_access` | `CALLCODE` | `target_coef` | `param_account_mode=AccountMode.EXISTING_CONTRACT_SAME_MAX` / `param_opcode=CALLCODE` | `geth` (R²) |
| `test_account_access` | `DELEGATECALL` | `target_coef` | `param_account_mode=AccountMode.EXISTING_CONTRACT_MINIMAL` / `param_opcode=DELEGATECALL` | `geth` (R²) |
| `test_account_access` | `DELEGATECALL` | `target_coef` | `param_account_mode=AccountMode.EXISTING_CONTRACT_SAME_MAX` / `param_opcode=DELEGATECALL` | `geth` (R²) |
| `test_account_access` | `EXTCODECOPY` | `target_coef` | `param_account_mode=AccountMode.EXISTING_CONTRACT_MINIMAL` / `param_opcode=EXTCODECOPY` | `geth` (R²) |
| `test_account_access` | `EXTCODECOPY` | `target_coef` | `param_account_mode=AccountMode.EXISTING_CONTRACT_SAME_MAX` / `param_opcode=EXTCODECOPY` | `geth` (R²) |
| `test_account_access` | `EXTCODEHASH` | `target_coef` | `param_account_mode=AccountMode.EXISTING_CONTRACT_MINIMAL` / `param_opcode=EXTCODEHASH` | `geth` (R²) |
| `test_account_access` | `EXTCODEHASH` | `target_coef` | `param_account_mode=AccountMode.EXISTING_CONTRACT_SAME_MAX` / `param_opcode=EXTCODEHASH` | `geth` (R²) |
| `test_account_access` | `EXTCODESIZE` | `target_coef` | `param_account_mode=AccountMode.EXISTING_CONTRACT_MINIMAL` / `param_opcode=EXTCODESIZE` | `geth` (R²) |
| `test_account_access` | `EXTCODESIZE` | `target_coef` | `param_account_mode=AccountMode.EXISTING_CONTRACT_SAME_MAX` / `param_opcode=EXTCODESIZE` | `geth` (R²) |
| `test_account_access` | `STATICCALL` | `target_coef` | `param_account_mode=AccountMode.EXISTING_CONTRACT_MINIMAL` / `param_opcode=STATICCALL` | `geth` (R²) |
| `test_account_access` | `STATICCALL` | `target_coef` | `param_account_mode=AccountMode.EXISTING_CONTRACT_SAME_MAX` / `param_opcode=STATICCALL` | `geth` (R²) |

</details>

<details>
<summary><code>COLD_ACCOUNT_CODE_WRITE</code> — 2 weak combos</summary>

| Test | Target opcode | Coef | Combo | Failing clients |
| --- | --- | --- | --- | --- |
| `test_account_access` | `CALLCODE` | `target_coef` | `param_account_mode=AccountMode.EXISTING_CONTRACT_MINIMAL` | `geth` (R²) |
| `test_account_access` | `CALLCODE` | `target_coef` | `param_account_mode=AccountMode.EXISTING_CONTRACT_SAME_MAX` | `geth` (R²) |
