...
- **Change the op set:** edit `models.presets` in `fit.yaml` (preset names; 104 available in evm-gasfit 0.3.0). New 8038 gas params that error as missing in the osaka table go under `new_params` (this repo already sets `COLD_ACCOUNT_*`, `ACCOUNT_WRITE`, `STORAGE_WRITE`, plus the `derived` block).
...
- **Custom model specs:** `models.presets` takes preset *names* only — no override mechanism; to tweak a preset you'd copy it into `models.custom` and drop the name from `presets`. This repo doesn't currently use it: it held custom copies of `cold_account_code_access` / `cold_account_code_write` (adding `!AccountMode.EXISTING_CONTRACT_JUMPDEST` to `filter_by`, since that fixture variant pointed the target opcode at a single-JUMPDEST contract and was winning the worst-case selection) until an evm-gasfit bump excluded that fixture natively, at which point `fit.yaml` reverted to the plain preset names. `build_site.py` still labels any custom specs `custom: <target_coef>` on the methodology param map (`_model_source_label`), gated on `param_map.has_custom` — that only fires when viewing archived runs from the window when the custom copies were in `fit.yaml` (see `data/runs/`).
...
- A run dir holds `meta.json`, `fit.yaml`, the markdown reports, the CSVs, and `figs/{runtime,glue}/`. **Proposal figs are not archived** — both `heatmap.png` and the per-param `provenance__*.png` are rendered as themed HTML tables (`build_proposal_heatmap` / `build_provenance_heatmap` in `build_site.py`, from `new_gas_all_params.csv`), so `archive_run.py` drops `figs/proposal/` on copy and `.gitignore` excludes the (still-emitted) `data/gasfit/figs/proposal/`.
- **`make gasfit` runs evm-gasfit THEN `scripts/archive_run.py`** — archive_run snapshots `data/gasfit/` (minus `figs/proposal/`) + `data/raw/meta.json` + `fit.yaml` into the run dir, then prunes `figs/runtime/` + `figs/glue/` from every run beyond the 3 newest (`KEEP_FULL_FIGS`). So those older runtime/glue pages render text/tables but no plots.
...
- `new-gas` — wholesale render of `new_gas_proposal.md` (minus H1 + Contents), except the proposal table is re-rendered (Δ-colored) and every `figs/proposal/*.png` is swapped (via sentinel tokens) for a themed HTML heatmap table built from `new_gas_all_params.csv`: the `## Client comparison` `heatmap.png` → `build_proposal_heatmap`; each `## Worst-case provenance` `provenance__<param>.png` → `build_provenance_heatmap` (rows = candidate combos in the report's `M1…Mn` order, winner cell outlined). Both fall back to the PNG only if the CSV is absent — and the PNGs are no longer archived, so that fallback is effectively dead.
