...
- **`make gasfit` runs evm-gasfit THEN `scripts/archive_run.py`** — archive_run snapshots `data/gasfit/` (minus `figs/proposal/`) + `data/raw/meta.json` + `fit.yaml` into the run dir, then prunes `figs/runtime/` + `figs/glue/` from every run beyond the 5 newest. So those older runtime/glue pages render text/tables but no plots.
