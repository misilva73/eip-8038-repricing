...
- **Custom model specs:** `models.presets` takes preset *names* only — no override mechanism. To tweak a preset you must copy it into `models.custom` and drop the name from `presets`. This repo does that for `cold_account_code_access` / `cold_account_code_write`, adding `!AccountMode.EXISTING_CONTRACT_JUMPDEST` to `filter_by` (that fixture variant points the target opcode at a single-JUMPDEST contract — not a representative cold account — and it was winning the worst-case selection). **These copies drift silently on an evm-gasfit bump**: re-diff them against `PRESETS` in `evm_gasfit/defaults/models.py` after upgrading. `build_site.py` labels custom specs `custom: <target_coef>` on the methodology param map (`_model_source_label`), and the JUMPDEST caveat there is gated on `param_map.has_custom` so pre-change archived runs don't claim it.
